...
1. **拉取代码库**：人类开发者正常通过 `git clone` 或 `git pull` 获取最新的项目代码（包含 `docs` 和 `.ai-collab` 目录）。
2. **配置 Obsidian 客户端**：
   - 下载并安装 [Obsidian](https://obsidian.md/)。
   - 点击 **“打开文件夹作为仓库 (Open folder as vault)”**，选择**项目根目录**（`vibe-coding-project`）。
   - *（建议）* 在 Obsidian 的“选项 -> 文件与链接 -> 排除文件”中，填入代码目录（如 `src/, node_modules/` 等），这样可以防止 Obsidian 去检索大量代码文件导致卡顿，让它专注管理 `docs` 和 `.ai-collab`。
...
## 5. 推荐的 Obsidian Skills (第三方插件) 装备库

为了让团队和 AI 能更好地利用这个知识库，强烈推荐所有成员（人类）在 Obsidian 中安装以下核心插件（在 Obsidian 设置 -> 第三方插件 -> 社区插件市场中搜索）：

| 插件名称 (Skill) | 作者 | 核心作用 |
| :--- | :--- | :--- |
| **Obsidian Git** | Denis Olegov | **【必装】** 团队记忆的“同步引擎”。每隔 10 分钟在后台静默完成 `git pull` 和 `git commit/push`。 |
| **Dataview** | Michael Brenan | **【推荐】** 知识库的“SQL 引擎”。能用代码语法实时汇总带有 `#task` 标签的日志，或状态为 `status: in_progress` 的特性文档。 |
| **Excalidraw** | Zsolt Viczian | **【强烈推荐】** 架构设计的“画板”。可以直接在 Obsidian 里画系统架构图，且文件会保存为 `.md` 格式，AI 也能读懂其中的文本节点！ |
| **Linter** | Victor Tao | **【推荐】** 格式化工具。团队写完文档后一键排版，保证 Markdown 格式严谨一致，防止无意义的 Git 冲突。 |
| **Templater** | SilentVoid13 | **【进阶】** 模板引擎。我们未来可以把 `ADR (架构决策)`、`AI Agent Prompt` 的模板放进去，按个快捷键就能生成标准格式。 |

### 5.1 专为 AI Agent 打造的专属 Skills：`kepano/obsidian-skills`
除了上述给人类用的插件，最近 Obsidian 官方 CEO Kepano 开源了一个专给 AI 用的神级仓库：**`kepano/obsidian-skills`**。

**【强烈建议引入】** 这套 Skills 完美契合我们项目使用 Claude Code 等 CLI Agent 的场景。
- **它的作用**：教你的 AI 助手（如 Claude Code）如何原生读写 Obsidian 特有的格式。
- **包含的核心能力**：
  - `obsidian-markdown`：教 AI 写双向链接 `[[ ]]` 和 YAML 属性。
  - `json-canvas`：教 AI 如何生成和修改白板（Canvas）文件，用于梳理架构图。
  - `obsidian-cli`：教 AI 如何通过命令行操控你的 Obsidian 库。

**如何让本项目的 AI 装备它：**
在终端执行以下命令，将这套技能克隆到本地 Claude Code 或 OpenCode 的技能库中：
```bash
# 针对 OpenCode 示例
git clone https://github.com/kepano/obsidian-skills.git ~/.opencode/skills/obsidian-skills
```
*装备后，您的 AI 员工就正式拥有了操作 Obsidian 的母语能力！*

> **提示**：如果您的网络无法直接访问 Obsidian 插件市场，您也可以在 GitHub 搜索对应插件的 Releases 页面，下载 `main.js`, `manifest.json` 和 `styles.css`，丢到项目下的 `.obsidian/plugins/插件名/` 目录中。我们把这些插件文件随 Git 提交后，**其他团队成员拉取代码就能直接用，无需重复下载！**
